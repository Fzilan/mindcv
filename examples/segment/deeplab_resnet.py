...
class DilatedBottleneck(Bottleneck):
    ...
    def __init__(
        ...
        self.bn1 = norm(width, use_batch_statistics=use_batch_statistics)
        self.conv2 = conv3x3(channels, channels, stride=stride, 
                             dilation=dilation, padding=dilation)
        self.bn2 = norm(width, use_batch_statistics=use_batch_statistics)
...
class DilatedResNet(ResNet):
    ...
    def __init__(
        ...
        self.bn1 = norm(self.input_channels, use_batch_statistics=use_batch_statistics)
        self.relu = nn.ReLU()
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, use_batch_statistics=use_batch_statistics)
    ...
    def _make_layer(
        ...
        if stride != 1 or self.input_channels != channels * block.expansion:
            down_sample = nn.SequentialCell([
                ...
                self.norm(channels * block.expansion, use_batch_statistics=use_batch_statistics)
            ])
