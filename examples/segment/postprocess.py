...
def preprocess_(img_lst, args, crop_size=513):
    ...
    for l in range(args.batch_size):
        ...
        img_, resize_h, resize_w = image_preprocess(image=img_,
                                                   crop_size=crop_size,
                                                   args=args)
        batch_img[l] = img_
...
def eval_batch(args, eval_net, batch_img, resize_hw, origin_hw, flip=True):  
    ...
    # resize to origin
    result_lst = resize_to_origin(input=net_out, 
                                  origin_hw=origin_hw, 
                                  resize_hw=resize_hw, 
...
def eval_batch_scales(args, eval_net, img_lst, scales,
    ...
    batch_img, resize_hw, origin_hw = preprocess_(img_lst, args, crop_size=sizes_[0])
    probs_lst = eval_batch(args, eval_net, batch_img, resize_hw, origin_hw , flip=flip)
    
    for crop_size_ in sizes_[1:]:
        batch_img, resize_hw, origin_hw = preprocess_(img_lst, args, crop_size=crop_size_)
...
def apply_eval(eval_param_dict):
    ...
    args = args_.copy()
    args.batch_size = check_batch_size(len(img_lst), 
                                       ori_batch_size=args_.batch_size, 
                                       refine=True)
    ...
    for i, line in enumerate(img_lst):
        ...
        if inner_batch_idx == args.batch_size:
            ...
            hist += calculate_batch_hist(labels=batch_msk_lst, 
                                         preds=batch_res, 
                                         batch_size=args.batch_size,
